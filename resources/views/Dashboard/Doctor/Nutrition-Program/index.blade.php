        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <a href="{{ route('Doctor.NutritionPrograms.create') }}" class="btn btn-primary" role="button"
                                ...
                                @foreach ($nutritionPrograms as $nutritionProgram)
                                    <tr>
                                        ...
                                        <td><input type="checkbox" name="delete_select" id="example-select-all"
                                                value="{{ $nutritionProgram->id }}"></td>
                                        <td><a
                                                href="{{ route('Doctor.Users.show', $nutritionProgram->user->id) }}"><b>{{ $nutritionProgram->user->name }}</b></a>
                                        </td>
                                        <td>{{ $nutritionProgram->name }}</td>
                                        <td>{{ \Str::limit($nutritionProgram->description, 50) }}</td>
                                        <td>
                                            {{ $nutritionProgram->healthCategory == null ? '-' : $nutritionProgram->healthCategory->name }}
                                        </td>
                                        <td>
                                            {{ $nutritionProgram->illness == null ? '-' : $nutritionProgram->illness->name }}
                                        </td>
                                        ...
                                        <td>{{ $nutritionProgram->created_at->diffforhumans() }}</td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu tx-13">
                                                    ...
                                                    <a class="dropdown-item"
                                                        href="{{ route('Doctor.NutritionPrograms.edit', $nutritionProgram->id) }}">
                                                        <i style="color: #0ba360"
                                                            class="text-success ti-user"></i>&nbsp;&nbsp;Edit
                                                    </a>
                                                    <!-- رابط لحذف بيانات -->
                                                    <a class="dropdown-item" href="#" data-toggle="modal"
                                                        data-target="#delete{{ $nutritionProgram->id }}">
                                                        <i class="text-danger ti-trash"></i>&nbsp;&nbsp;Delete data
                                                    </a>
                                                    <!-- رابط لعرض بيانات -->
                                                    <a class="dropdown-item"
                                                        href="{{ route('Doctor.NutritionPrograms.show', $nutritionProgram->id) }}">
